...
namespace Mob
{
	public class MobNetworkBehaviour : NetworkBehaviour
	{
		[NonSerialized]
		...
		public void FlushAll(){
			if (!isServer)
				return;
			if (timeToDestroy > -1f) {
				StartCoroutine (AllFlushing (gameObject, timeToDestroy));
			}
		}

		IEnumerator AllFlushing(GameObject gameObject, float timeToDestroy){
